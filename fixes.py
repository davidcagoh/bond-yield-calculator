# Fixed code snippets for the three critical issues

# ============================================================================
# FIX 1: Spot Curve Bootstrapping (with proper interpolation)
# ============================================================================
...
# ============================================================================
# FIX 2: Forward Curve (using continuous compounding formula)
# ============================================================================
...
# ============================================================================
# FIX 3: PCA Analysis (moved after covariance matrices, with proper variable definitions)
# ============================================================================
...
# --- Yields log returns ---
# Formula: X_i,j = log(r_i,j+1 / r_i,j) for j=1...9 (10 days means 9 log returns)

# Pivot YTM to have columns = T (maturity in years), rows = Date
ytm_pivot = result_df.pivot(index='Date', columns='T', values='YTM').sort_index()

# Daily log returns: X_i,j = log(r_i,j+1 / r_i,j)
# shift(1) shifts down, so ytm_pivot / ytm_pivot.shift(1) gives r_j+1 / r_j
ytm_logret = np.log(ytm_pivot / ytm_pivot.shift(1)).dropna()

# Covariance matrix
cov_yields = ytm_logret.cov()

# --- Forward rates log returns ---
# Select only forward rates starting at 1yr (1yr→2yr, 1yr→3yr, 1yr→4yr, 1yr→5yr)
fwd_filtered = forward_df[forward_df['Start']==1]

# Pivot to have columns = End (forward end in years), rows = Date
fwd_pivot = fwd_filtered.pivot(index='Date', columns='End', values='Forward_Rate').sort_index()

# Daily log returns: X_i,j = log(r_i,j+1 / r_i,j)
fwd_logret = np.log(fwd_pivot / fwd_pivot.shift(1)).dropna()

# Covariance matrix
cov_forward = fwd_logret.cov()
...
print("Fixed code snippets created. Use these to update the notebook cells.")
